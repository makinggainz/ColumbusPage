...
export const BottleScene = ({ onBottleClick, visible, dark }: { onBottleClick?: () => void; visible?: boolean; dark?: boolean }) => {
  ...
  const draw = useCallback(() => {
    ...
    ctx.fillStyle = dark ? "#000000" : "#F9F9F9";
    ...
    for (let r = 0; r < gridRows; r++) {
      ...
      ctx.strokeStyle = dark ? `rgba(100,180,255,${Math.max(0, alpha).toFixed(3)})` : `rgba(20,60,160,${Math.max(0, alpha).toFixed(3)})`;
    ...
    ctx.strokeStyle = dark ? "rgba(100,180,255,0.06)" : "rgba(20,60,160,0.06)";
    ctx.lineWidth = 14;
    ctx.stroke();
    ...
    if (hoveringBottle) {
      ...
      if (hintP) {
        ...
        ctx.fillStyle = dark ? "rgba(100,180,255,0.5)" : "rgba(20,60,160,0.5)";
        ctx.textAlign = "center";
