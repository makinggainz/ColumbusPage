...
// Animated 3D-perspective wave mesh, ported from the experimentV6 home hero
// (components/home/Hero.tsx → `WaveMesh`). All pointer interactivity from the
// original — drag-the-boat, cursor ripples, mouse-proximity wave push, cursor
// swaps — has been removed: this is a purely ambient background. The boat
// still auto-sails rightward and sheds wake ripples on its own.
...
/* ── Wave height (base ocean, no ripples) ── */
...
function drawBoat3D(
  ...
  const hullStroke = `rgba(20,81,232,${(0.5 * a).toFixed(3)})`;
  const hullFill = `rgba(20,81,232,${(0.06 * a).toFixed(3)})`;
  const hullFillDark = `rgba(20,81,232,${(0.1 * a).toFixed(3)})`;
  const ribStroke = `rgba(20,81,232,${(0.25 * a).toFixed(3)})`;
  const mastStroke = `rgba(20,81,232,${(0.45 * a).toFixed(3)})`;
  const sailFill = `rgba(20,81,232,${(0.05 * a).toFixed(3)})`;
  const sailStroke = `rgba(20,81,232,${(0.25 * a).toFixed(3)})`;
  const riggingStroke = `rgba(20,81,232,${(0.15 * a).toFixed(3)})`;
  const flagFill = `rgba(20,81,232,${(0.3 * a).toFixed(3)})`;
  const scStroke = `rgba(20,81,232,${(0.4 * a).toFixed(3)})`;
  const scFill = `rgba(20,81,232,${(0.08 * a).toFixed(3)})`;
  ...
  // ── Crew ──
  const crewStroke = `rgba(20,81,232,${(0.5 * a).toFixed(3)})`;
  const crewHead = `rgba(20,81,232,${(0.35 * a).toFixed(3)})`;
  const headR = 0.9;
  const bodyH = 3;
...
/* ── 3D Perspective Wave Mesh (ambient, non-interactive) ── */
export function TechHeroMesh() {
  ...
  useEffect(() => {
    ...
    // `draw` loop can reference itself without a self-referencing
    // useCallback (which the react-hooks lint rule rejects).
    let animId = 0;
    let visible = true;
    ...
    const draw = () => {
      ...
      // ── Boat physics — auto-sail only (drag interactivity removed) ──
      ...
      // Full wave height including ripples (boat wake only — no mouse push)
      ...
      for (let c = 0; c < gridCols; c++) {
        ...
        for (let r = gridRows - 1; r >= 0; r--) {
          ...
          ctx.strokeStyle = `rgba(20,81,232,${(0.08 + depthT * 0.22).toFixed(3)})`;
