...
function resolveCircleRect(b: Box, obs: Rect) {
  const r = b.size / 2;
  const left = obs.x - OBS_PAD; const top = obs.y - OBS_PAD;
  const right = obs.x + obs.w + OBS_PAD; const bottom = obs.y + obs.h + OBS_PAD;
...
export const TravelPromo = () => {
  ...
  useEffect(() => {
    imagesRef.current = EMOJI_DEFS.map((def) => {
      const img = new window.Image(); img.src = def.src; return img;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    EMOJI_DEFS.forEach((def, i) => {
      boxesRef.current.push({
        ...
        size: def.size, img: imagesRef.current[i], spawnTime: Date.now(),
      });
    });
  }, [progress]);
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (x >= 0 && x <= rect.width && y >= 0 && y <= rect.height) mouseRef.current = { x, y, active: true };
      else mouseRef.current.active = false;
    };
    ...
    const loop = () => {
      ...
      for (const el of [titleRef.current, subtitleRef.current, ctaRef.current]) {
        ...
        obstacles.push({ x: r.left - canvasRect.left, y: r.top - canvasRect.top, w: r.width, h: r.height });
      }
      const boxes = boxesRef.current; const mouse = mouseRef.current;
      for (const b of boxes) {
        b.vx *= FRICTION; b.vy *= FRICTION; b.angularVel *= ANG_FRICTION;
        if (mouse.active) {
        ...
        for (const obs of obstacles) resolveCircleRect(b, obs);
      }
      ...
      for (const b of boxes) {
        ...
        const age = now - b.spawnTime; const alpha = Math.min(1, age / FADE_IN_MS);
  ...
  const fadeUp = (p: number, dist = 32) => ({
    opacity: p, filter: `blur(${(1 - p) * 8}px)`, transform: `translateY(${(1 - p) * dist}px)`,
  });
          ...
          <div ref={containerRef} className="sticky top-0 h-screen overflow-hidden bg-white">
            ...
            <div className="relative h-full">
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 pointer-events-none" style={{ zIndex: 2 }}>
                <h2 ref={titleRef} className="text-[48px] md:text-[56px] font-semibold tracking-[-0.003em] leading-[1.07] text-[#1D1D1F] mb-4" style={fadeUp(titleP)}>
                  Love to travel or go out?
                </h2>
                <p ref={subtitleRef} className="text-[21px] md:text-[24px] font-normal leading-[1.38] text-[#6E6E73] max-w-[680px]" style={fadeUp(subtitleP)}>
                  <span className="font-semibold text-[#1D1D1F]">MapsGPT</span>{" "}is already answering thousands of queries in your area
                </p>
                <div ref={ctaRef} className="mt-6 pointer-events-auto" style={fadeUp(ctaP)}>
